...
function dashCalcMetricas(asesor, mes, anio) {
  const ventasFiltradas = DASH_STATE.ventas.filter(v => {
    ...
    const vAsesor = String(v.Asesor || '').toUpperCase().trim();
    return vMes === mes && vAnio === anio && vAsesor === asesor.toUpperCase().trim();
  });
  ...
  const presupFiltrado = DASH_STATE.presupuesto.filter(p => {
    const pAsesor = String(p.Asesor || '').toUpperCase().trim();
    const pMes = parseInt(p.Mes);
    const pAnio = parseInt(p.Año || p.Anio || p['Año'] || 0);
    return pAsesor === asesor.toUpperCase().trim() && pMes === mes && pAnio === anio;
  });
...
function dashGetAsesores() {
  const set = new Set(DASH_STATE.ventas.map(v => String(v.Asesor || '').trim()).filter(Boolean));
  return [...set];
}

// ── Calcular clientes en riesgo ──────────────────────────────────────────────
function dashClientesEnRiesgo(asesor, diasUmbral = 60) {
  // Agrupar última compra por cliente para este asesor
  const ultimaCompra = {};
  DASH_STATE.ventas
    .filter(v => !asesor || String(v.Asesor || '').toUpperCase().trim() === asesor.toUpperCase().trim())
    .forEach(v => {
...
function dashResumenAnualHtml(hist, anio) {
  ...
  return `
    <div class="dash-annual">
      ...
      <canvas id="dash-chart-hist" style="margin-top:12px;width:100%;height:120px" role="img" aria-label="Histórico mensual de ventas"></canvas>
    </div>
  `;
}
